import templateText from './duck_svg_template'

export class DuckModal {
  ...
  generate() {
    ...
    return this.initialization().replaceAll(/[\n]/g, '');
  }

  private getEggShapePoints(a, b, k, isFace, segment_points) {
    ...
    for (var i = 0; i < segment_points; i++) {
      ...
      var x =
        ...
        this.randomFromInterval(-a / 200.0, a / 200.0);
      // pointString += x + "," + y + " ";
      result.push([x, y]);
    }
    for (var i:number = segment_points; i > 0; i--) {
    ...
    for (var i = 0; i < segment_points; i++) {
      ...
      if (isFace) {
        if (i == mouthSelectionSlice) {
          ...
          this.mouthY = trueY;
          this.mouthX = -trueX;
          this.mouthWidth = this.randomFromInterval(0.4 * -trueX, 0.5 * -trueX);
          this.mouthHeight = this.randomFromInterval(
            0.3 * this.mouthWidth,
            0.5 * this.mouthWidth
          );
        ...
        if (i == eyeSelectionSlice) {
          ...
          this.eyePositionScale = this.randomFromInterval(2.9, 4.1);
          ...
          this.faceRotateDegree = this.randomFromInterval(-80, 80);
  ...
  private initializeFace() {
    this.faceY = this.randomFromInterval(15, 30);
    this.faceX = this.randomFromInterval(10, 18);
    this.faceScale = this.randomFromInterval(1.0, 1.2);
    this.faceK =
      this.randomFromInterval(0.001, 0.01) * (Math.random() > 0.5 ? 1 : -1);
    ...
    this.faceRotation = this.randomFromInterval(-30, 30);
    this.faceTranslationX = this.randomFromInterval(
      -this.faceX / 6,
      this.faceX / 6
    );
    this.faceTranslationY = this.randomFromInterval(
      -this.faceY / 4,
      this.faceY / 4
    );
  }
  ...
  private initializeBody() {
    this.bodyY = this.randomFromInterval(20, 30);
    this.bodyX = this.randomFromInterval(20, 30);
    this.bodyScale = this.randomFromInterval(1.0, 1.2);
    this.faceK =
      this.randomFromInterval(0.0001, 0.001) * (Math.random() > 0.5 ? 1 : -1);
    this.bodyRotation = this.randomFromInterval(-90, 90);
    ...
    this.bodyTranslationY =
      this.bodyY + this.randomFromInterval(-this.bodyY / 6, this.bodyY / 6);
    this.bodyTranslationX = this.randomFromInterval(
      -this.bodyX / 8,
      this.bodyX / 8
    );
  ...
  private initializeLeg() {
    ...
    var leftLegRotateDegree = this.randomFromInterval(
      -this.faceRotateDegree / 4,
      this.faceRotateDegree / 1.1
    );
    var rightLegRotateDegree = this.randomFromInterval(
      -this.faceRotateDegree / 4,
      this.faceRotateDegree / 1.1
    );
    while (
      ...
      leftLegRotateDegree = this.randomFromInterval(
        -this.faceRotateDegree / 4,
        this.faceRotateDegree / 1.1
      );
      rightLegRotateDegree = this.randomFromInterval(
        -this.faceRotateDegree / 4,
        this.faceRotateDegree / 1.1
      );
    }
    ...
    for (var i = 0; i < rotatedPoints.length; i++) {
      ...
      if (rotatedPoints[i][1] > 0) {
        ...
        if (Math.pow(y / x - leftLegTan, 2) < leftLegClosestDistance) {
          ...
          leftLegClosestPoint = [
            x * this.randomFromInterval(0.5, 0.8),
            y * this.randomFromInterval(0.5, 0.8),
          ];
        }
        ...
        if (Math.pow(y / x - leftLegEndTan, 2) < leftLegEndClosestDistance) {
          ...
          leftLegEndClosestPoint = [
            x * this.randomFromInterval(1.2, 1.3),
            y * this.randomFromInterval(1.2, 1.3),
          ];
        }
        if (
    ...
    var leftControlRandom1 = this.randomFromInterval(0.2, 0.4);
    var leftControlRandom2 = this.randomFromInterval(0.2, 0.4);
    var leftLegNormal = [
    ...
    var rightControlRandom1 = this.randomFromInterval(0.2, 0.4);
    var rightControlRandom2 = this.randomFromInterval(0.2, 0.4);
    var rightLegNormal = [
  ...
  private initializeFeet() {
    ...
    this.feetLength =
      ...
      ) * this.randomFromInterval(0.4, 0.6);
    // initialize left feet middle finger
    this.leftFeetMiddleFingerPoints =
      "0,0 0," + -this.feetLength * this.randomFromInterval(0.8, 1.1);
