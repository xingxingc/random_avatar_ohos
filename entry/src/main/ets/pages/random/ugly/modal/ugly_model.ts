import { generateBothEyes } from "./utils/eye_shape"
import { generateFaceCountourPoints } from "./utils/face_shape"
import { generateHairLines0, generateHairLines1, generateHairLines2, generateHairLines3 } from "./utils/hair_lines"
import { generateMouthShape0, generateMouthShape1, generateMouthShape2 } from "./utils/mouth_shape"
import templateText from './ugly_face_svg_template'

export class UglyModel {
  ...
  generateFace() {
    ...
    let eyes = generateBothEyes(this.faceWidth / 2);
    let left = eyes.left;
    ...
    this.distanceBetweenEyes =  this.randomFromInterval(
      this.faceWidth / 4.5,
      this.faceWidth / 4,
    );
    this.eyeHeightOffset =  this.randomFromInterval(
      this.faceHeight / 8,
      this.faceHeight / 6,
    );
    this.leftEyeOffsetX =  this.randomFromInterval(
      -this.faceWidth / 20,
      this.faceWidth / 10,
    );
    this.leftEyeOffsetY =  this.randomFromInterval(
      -this.faceHeight / 50,
      this.faceHeight / 50,
    );
    this.rightEyeOffsetX =  this.randomFromInterval(
      -this.faceWidth / 20,
      this.faceWidth / 10,
    );
    this.rightEyeOffsetY =  this.randomFromInterval(
      -this.faceHeight / 50,
      this.faceHeight / 50,
    );
    this.leftEyeCenter = left.center[0];
    this.rightEyeCenter = right.center[0];
    this.leftPupilShiftX =  this.randomFromInterval(
      -this.faceWidth / 20,
      this.faceWidth / 20,
    );
    ...
    let leftInd0 = Math.floor( this.randomFromInterval(10, left.upper.length - 10));
    let rightInd0 = Math.floor(
       this.randomFromInterval(10, right.upper.length - 10),
    );
    let leftInd1 = Math.floor( this.randomFromInterval(10, left.upper.length - 10));
    let rightInd1 = Math.floor(
       this.randomFromInterval(10, right.upper.length - 10),
    );
    let leftLerp =  this.randomFromInterval(0.2, 0.8);
    let rightLerp =  this.randomFromInterval(0.2, 0.8);
    ...
    let numHairLines = [];
    let numHairMethods = 4;
    for (let i = 0; i < numHairMethods; i++) {
      numHairLines.push(Math.floor( this.randomFromInterval(0, 50)));
    }
    this.hairs = [];

    if (Math.random() > 0.3) {
      this.hairs = generateHairLines0(
    ...
    this.rightNoseCenterX =  this.randomFromInterval(
      this.faceWidth / 18,
      this.faceWidth / 12,
    );
    this.rightNoseCenterY =  this.randomFromInterval(0, this.faceHeight / 5);
    this.leftNoseCenterX =  this.randomFromInterval(
      -this.faceWidth / 18,
      -this.faceWidth / 12,
    );
    this.leftNoseCenterY =
      this.rightNoseCenterY +
       this.randomFromInterval(-this.faceHeight / 30, this.faceHeight / 20);
    if (Math.random() > 0.1) {
      // use natural hair color
      this.hairColor = this.hairColors[Math.floor(Math.random() * 10)];
    } else {
      this.hairColor = "url(#rainbowGradient)";
      this.dyeColorOffset =  this.randomFromInterval(0, 100) + "%";
    }

    let choice = Math.floor(Math.random() * 3);
    if (choice == 0) {
      this.mouthPoints = generateMouthShape0(
        this.computedFacePoints,
        this.faceHeight,
        this.faceWidth,
      );
    } else if (choice == 1) {
      this.mouthPoints = generateMouthShape1(
        this.computedFacePoints,
        this.faceHeight,
        this.faceWidth,
      );
    } else {
      this.mouthPoints = generateMouthShape2(
        this.computedFacePoints,
        this.faceHeight,
        this.faceWidth,
      );
