...
export function generateRectangularFaceContourPoints(a, b, segment_points) {
  ...
  for (let i = 0; i < segment_points; i++) {
    ...
    let intersection = findIntersectionPoints(degree, a, b);
...
export function generateFaceCountourPoints(numPoints = 100) {
  let faceSizeX0 = randomFromInterval(50, 100);
  let faceSizeY0 = randomFromInterval(70, 100);

  let faceSizeY1 = randomFromInterval(50, 80);
  let faceSizeX1 = randomFromInterval(70, 100);
  let faceK0 =
    randomFromInterval(0.001, 0.005) * (Math.random() > 0.5 ? 1 : -1);
  let faceK1 =
    randomFromInterval(0.001, 0.005) * (Math.random() > 0.5 ? 1 : -1);
  let face0TranslateX = randomFromInterval(-5, 5);
  let face0TranslateY = randomFromInterval(-15, 15);

  let face1TranslateY = randomFromInterval(-5, 5);
  let face1TranslateX = randomFromInterval(-5, 25);
  let eggOrRect0 = Math.random() > 0.1;
  let eggOrRect1 = Math.random() > 0.3;

  let results0 = eggOrRect0
